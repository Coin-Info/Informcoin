...
#if defined(NDEBUG)
# error "Informcoin cannot be compiled without assertions."
#endif
...
uint256 hashGenesisBlock("0xb88da28280668e5dac0a1abc8a73ccc8d0f7a1d538b6203ad6057727c64a5791");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 1); // Informcoin: starting difficulty is 1 / 2^26
...
int64 CTransaction::nMinTxFee = 5000;
/** Fees smaller than this (in satoshi) are considered zero fee (for relaying) */
int64 CTransaction::nMinRelayTxFee = 5000;
...
const string strMessageMagic = "Informcoin Signed Message:\n";
...
{
    // Informcoin: IsDust() detection disabled, allows any valid dust to be relayed.
                              ...
                              enum GetMinFee_mode mode) const
{
    ...
    int64 nMinFee = (1 + (int64)nBytes / 20000) * nBaseFee;
...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
    ...
    // Subsidy is cut to half after every 31536000 blocks, which will occur in approximately every year
    nSubsidy >>= (nHeight / 31536000); // Informcoin: ~31536000 blocks in a year

    return nSubsidy + nFees;
}

static const int64 nTargetTimespan = 1 * 2; // Informcoin: 2 second
static const int64 nTargetSpacing = 1 * 1; // Informcoin: 1 second
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // Informcoin: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // Informcoin: Special short-term limits to avoid 10,000 BDB lock limit:
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0xb88da28280668e5dac0a1abc8a73ccc8d0f7a1d538b6203ad6057727c64a5791");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "UK Time 6AM 11 May 2018 Inform coin developers built informcoin";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        txNew.vin[0].scriptSig = CScript() << 486604799 << CBigNum(4) << vector<unsigned char>((const unsigned char*)pszTimestamp, (const unsigned char*)pszTimestamp + strlen(pszTimestamp));
        txNew.vout[0].nValue = 50 * COIN;
        txNew.vout[0].scriptPubKey = CScript() << ParseHex("041234507fa689ad5023690c80f3a49c8f13f8d45b8c857fbcbc8bc4a8e4d3eb4b10f4d4604fa08dce601aaf0f470216fe1b51850b4acf21b179c45070ac7b04b6") << OP_CHECKSIG;
        CBlock block;
        block.vtx.push_back(txNew);
        block.hashPrevBlock = 0;
        block.hashMerkleRoot = block.BuildMerkleTree();
        block.nVersion = 1;
        block.nTime    = 1526015302;
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = 1122626;
        ...
        assert(block.hashMerkleRoot == uint256("0xa0b4492cfc13e406b721ab48365ce9eb1e081ccd59216c4517a8bd7612d7d96c"));
...
unsigned char pchMessageStart[4] = { 0xa2, 0xb1, 0xa2, 0xdb }; // Informcoin: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("InformcoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("InformcoinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static InformcoinMiner(CWallet *pwallet)
{
    printf("InformcoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("informcoin-miner");
    ...
    try { loop {
        ...
        printf("Running InformcoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("InformcoinMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&InformcoinMiner, pwallet));
}
